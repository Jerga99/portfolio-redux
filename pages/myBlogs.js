...
class MyBlogs extends React.Component {
  ...
  deleteBlogConfirm(blogToDelete) {
    actions.deleteBlog(blogToDelete._id).then(
      () => {
        const { blogs } = this.state;
        const blogIndex = blogs.findIndex(blog => blog._id === blogToDelete._id);
  ...
  createAdditionalOptions = (blog, index) => {
    ...
    return [
      {text: blogState.view, handlers: { onClick: () => this.changeBlogState(blog, blogState) } },
